...
from pyscf.soscf.newton_ah import _gen_rhf_response_scaled
...
def gen_tda_operation_scaled(mf, sf, fock_ao=None, singlet=True, wfnsym=None):
    '''Generate function to compute (A+B)x
    ...
    mo_coeff = numpy.asarray(numpy.hstack((orbo,orbv)), order='F')
    vresp = _gen_rhf_response_scaled(mf, sf, singlet=singlet, hermi=0)
...
gen_tda_hop_scaled = gen_tda_operation_scaled
...
class TDA(lib.StreamObject):
    ...
    def gen_vind_scaled(self, mf, sf):
        '''Compute Ax'''
        return gen_tda_hop_scaled(mf, sf, singlet=self.singlet, wfnsym=self.wfnsym)
    ...
    def scaled_kernel(self, sf, x0=None, nstates=None):
        '''TDA diagonalization solver
        '''
        cpu0 = (time.clock(), time.time())
        self.check_sanity()
        ...
        vind, hdiag = self.gen_vind_scaled(self._scf, sf)
        precond = self.get_precond(hdiag)
...
def gen_tdhf_operation_scaled(mf, sf, fock_ao=None, singlet=True, wfnsym=None):
    '''Generate function to compute
...
class TDHF(TDA):
    ...
    def gen_vind_scaled(self, mf, sf):
        return gen_tdhf_operation_scaled(mf, sf, singlet=self.singlet, wfnsym=self.wfnsym)
    ...
    def scaled_kernel(self, sf, x0=None, nstates=None):
        '''Scaled TDHF diagonalization with non-Hermitian eigenvalue solver
        '''
        cpu0 = (time.clock(), time.time())
        self.check_sanity()
        self.dump_flags()
