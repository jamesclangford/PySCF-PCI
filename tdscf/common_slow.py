...
class TDMatrixBlocks(object):
    ...
    def tdhf_scaled_full_form(self, scale, *args, **kwargs):
        """
        ...
        """
        m = self.tdhf_scaled_primary_form(scale, *args, **kwargs)
        self.__check_primary_form__(m)
        if m[0] == "ab":
            return ab2full(*m[1:])
        elif m[0] == "full":
    ...
    def tdhf_scaled_mk_form(self, scale, *args, **kwargs):
        """
...
class TDERIMatrixBlocks(TDMatrixBlocks):
    ...
    def tdhf_scaled_primary_form(self, scale, *args, **kwargs):
        """
        ...
        """
        d = self.tdhf_diag(*args, **kwargs)
        a = d + 2 * scale * self["knmj"] - self["knjm"]
        b = 2 * scale * self["knmj"] - self["kjnm"]
        return "ab", a, b
...
def analyze(tdobj, verbose=None):
    ...
    e_ev = numpy.asarray(tdobj.e)
    e_wn = (numpy.asarray(tdobj.e)/nist.HARTREE2EV) * nist.HARTREE2WAVENUMBER
...
def scaled_kernel(eri, scale, driver=None, fast=True, nroots=None, **kwargs):
    """
    ...
    """
    if not isinstance(eri, TDMatrixBlocks):
        raise ValueError("The arguments must be ERI object")

    if fast:

        logger.debug1(eri.model, "Preparing scaled TDHF matrix (fast) ...")
        tdhf_mk, tdhf_k = eri.tdhf_scaled_mk_form(scale, **kwargs)
        logger.debug1(eri.model, "Diagonalizing a {} matrix with {} ...".format(
            'x'.join(map(str,tdhf_mk.shape)),
            "'{}'".format(driver) if driver is not None else "a default method",
        ))
        vals, vecs_x = eig(tdhf_mk, driver=driver, nroots=nroots, half=False)
        #print('Debug: scaled_kernel vecs_x ', vecs_x)
        vals = vals ** .5
        vecs_y = (.1 / vals)[numpy.newaxis, :] * tdhf_k.dot(vecs_x)
        vecs_u, vecs_v, = vecs_y + vecs_x, vecs_y - vecs_x
        return vals, numpy.concatenate((vecs_u, vecs_v), axis=0)
        ...
        logger.debug1(eri.model, "Preparing scaled TDHF matrix ...")
        m = eri.tdhf_scaled_full_form(scale, **kwargs)
        logger.debug1(eri.model, "Diagonalizing a {} matrix with {} ...".format(
            'x'.join(map(str, m.shape)),
            "'{}'".format(driver) if driver is not None else "a default method",
        ))
...
class TDBase(object):
    ...
    def __scaled_kernel__(self, scale, **kwargs):
        """Silent implementation of kernel which does not change attributes."""
        if self.eri is None:
            self.eri = self.ao2mo()
    ...
    def scaled_kernel(self, scale):
        ...
        Returns:
            Positive eigenvalues (eV) and eigenvectors.
        """
        self.e, self.xy = self.__scaled_kernel__(scale)
        self.e = self.e*nist.HARTREE2EV
        print('Excited State energies (eV) \n', self.e)
        return self.e, self.xy
