...
def analyze_str(mf, verbose=logger.DEBUG, with_meta_lowdin=WITH_META_LOWDIN,
            **kwargs):
    '''Analyze the given SCF object:  print orbital energies, occupancies;
    print orbital coefficients; Mulliken population analysis; Diople moment.
    '''
    ...
    if log.verbose >= logger.NOTE:
        ...
        for i,c in enumerate(mo_occ):
            log.note('MO #%-3d energy= %-18.15g occ= %g', i+MO_BASE,
                     mo_energy[i], c)
            return_str += '\n' + 'MO #%-3d energy= %-18.15g occ= %g' + str(i+MO_BASE) + str(mo_energy[i]) + str(c)
