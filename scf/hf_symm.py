...
def analyze(mf, verbose=logger.DEBUG, with_meta_lowdin=WITH_META_LOWDIN,
    ...
    mol = mf.mol
    if not mol.symmetry:
        print('DEBUG: if no mol.symmetry')
    ...
    if log.verbose >= logger.NOTE:
        ...
        if mol.groupname in ('SO3', 'Dooh', 'Coov'):
            ...
            return_str += 'TODO: total wave-function symmetry for %s' + str(mol.groupname)
        else:
            log.note('Wave-function symmetry = %s',
                     symm.irrep_id2name(mol.groupname, wfnsym))
            return_str += 'Wave-function symmetry = ' + str(symm.irrep_id2name(mol.groupname, wfnsym))
        log.note('occupancy for each irrep:  ' + (' %4s'*nirrep), *mol.irrep_name)
        log.note('                           ' + (' %4d'*nirrep), *noccs)
        log.note('**** MO energy ****')
        #return_str += '\noccupancy for each irrep:  ' + str(' %4s'*nirrep) + str(*mol.irrep_name)
        #return_str += '\n                           ' + str(' %4d'*nirrep) + str( *noccs)
        return_str += '\n**** MO energy ****'
        irname_full = {}
        ...
        for k, j in enumerate(orbsym):
            ...
            mo_str = 'MO #{} ({} #{}), energy= {} occ= {}'
            mo_str = mo_str.format(k + MO_BASE, irname_full[j], irorbcnt[j], mo_energy[k], mo_occ[k])
            return_str += '\n' + mo_str
    ...
    return return_str, pop_and_charge, dip
